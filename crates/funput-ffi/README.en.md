...
Sentence and word starts take **no handle**: they read the text the host hands over and answer about
the position it ends at.

```c
// A null or empty text is the start of the document, so the start of a sentence and of a word.
...
A soft keyboard is what needs them: it draws its own Shift key, so it has to know on every caret move
whether that key should be up, and it cannot keep a running answer — a caret also moves for a paste,
a tap elsewhere, or a field that opened with text already in it, none of which a keystroke explains.
`funput_starts_sentence` always applies the typing reading of a full stop, which treats a repeated
one as an abbreviation: `v.v. ` does not open a sentence while `TS. ` does. The full rules are in
`docs/features/auto-capitalize.md`. Not behind a feature, because iOS and Android build this crate
...
src/sentence/      # C API for sentence/word starts, handle-free, not feature-gated
