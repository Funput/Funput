...
fn vni_zero_removes_tone() {
    let (buffer, results) = crate::support::type_keys(InputMethod::Vni, "a1");
    assert_eq!(buffer, "á");

    // Type "a10": the trailing 0 deletes the tone, sending plain "a".
    let (buffer, results2) = crate::support::type_keys(InputMethod::Vni, "a10");
    assert_eq!(buffer, "a");
...
fn vni_zero_keeps_shape() {
    ...
    let (buffer, _) = crate::support::type_keys(InputMethod::Vni, "a610");
    assert_eq!(buffer, "â");
}

#[test]
fn telex_z_removes_tone() {
    let (buffer, _) = crate::support::type_keys(InputMethod::Telex, "asz");
    assert_eq!(buffer, "a");

    // việt → viêt (tone gone, ê kept).
    let (buffer, _) = crate::support::type_keys(InputMethod::Telex, "vieetjz");
    assert_eq!(buffer, "viêt");
}

#[test]
fn remove_tone_with_no_tone_is_literal() {
    let (buffer, _) = crate::support::type_keys(InputMethod::Vni, "a0");
    assert_eq!(buffer, "a0");
}
