...
#[test]
fn vni_d9_stroke() {
    let (buffer, results) = crate::support::type_keys(InputMethod::Vni, "d9");
    assert_eq!(buffer, "đ");
    assert_eq!(results[1].action, Action::Send);
    assert_eq!(results[1].backspace, 1);
...
fn vni_a11_revert() {
    ...
    assert_eq!(
        crate::support::type_keys_buffer(InputMethod::Vni, "a11"),
        "a1"
    );
}
...
fn vni_ng1_literal_ignored() {
    let (buffer, results) = crate::support::type_keys(InputMethod::Vni, "ng1");
...
fn vni_reposition_multi_char_output() {
    ...
    let (buffer, results) = crate::support::type_keys(InputMethod::Vni, "to1an");
