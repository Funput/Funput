//! Composition-state indicators: window title + cursor color via OSC, wrapped so
//! they survive any surrounding terminal multiplexer and reach the outer terminal.

use std::io::{self, Write};

/// Terminal multiplexer wrapping the session, if any. A bare OSC sequence is
/// swallowed by tmux/screen, so it must be wrapped in their passthrough form to
