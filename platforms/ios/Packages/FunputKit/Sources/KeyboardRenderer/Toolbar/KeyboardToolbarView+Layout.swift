...
extension KeyboardToolbarView {
    enum Metrics {
        ...
        /// Keeps the shared content region clear of the utility keys.
    ...
    /// Places the utility keys and the shared content region across the toolbar band.
    func layoutContents() {
        ...
        // same time, so they get the same frame. It runs from the band's leading edge —
        // flush with the first keycap of the rows below — to wherever the controls begin.
        let contentRegion = CGRect(
            x: 0,
            y: 0,
            width: max(0, trailing - Metrics.contentInset),
