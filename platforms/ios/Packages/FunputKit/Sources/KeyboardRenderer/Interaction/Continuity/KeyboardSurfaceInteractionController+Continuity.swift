...
extension KeyboardSurfaceInteractionController {
    ...
    /// and the geometry snapshot taken when it landed, so it still commits a key from the
    /// layout the user was typing on. Only what is tied to the *old* keycap views goes:
    /// highlights, the preview, and a pending alternate palette whose anchor frame no longer
    /// exists.
    ///
