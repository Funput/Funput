...
struct PersonalSuggestionIntegrationTests {
    ...
    func tokenAndAcceptance() {
        ...
        let document = SuggestionTestDocument()
        type("ban", coordinator: coordinator, document: document)
        #expect(coordinator.takePersonalSuggestionUpdate().prefix == "ban")
        #expect(coordinator.acceptSuggestion("bạn", replacing: "ban", document: document))
    ...
    func decomposedToken() {
        let coordinator = KeyboardInputCoordinator(inputMethod: .vni)
        let document = SuggestionTestDocument()
        type("A\u{0301}n", coordinator: coordinator, document: document)
    ...
    private func type(
        ...
        document: SuggestionTestDocument
    ) {
...
@MainActor
final class SuggestionTestDocument: KeyboardDocument {
    var text = ""
