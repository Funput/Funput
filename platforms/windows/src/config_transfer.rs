...
//!
//! Pure `serde`/`serde_json`, no `windows`/`slint` deps, so it unit-tests on any
//! host. `preferences` + `shortcuts` are portable across platforms; Windows-only
//! data (hotkey preset, excluded exe list) lives under `platform.windows`. Other
//! platforms' blocks (e.g. `macos`) are ignored on import — serde skips unknown
//! keys — so a file exported on macOS still imports its portable parts here.
...
/// Only the Windows block is modelled. Unknown platform keys (e.g. `macos`) are
...
/// UTC ISO-8601 timestamp without pulling in a date crate (Howard Hinnant's
